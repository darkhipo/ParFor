...
Parser::ParseStatementOrDeclaration(bool OnlyStatement) {
  ...
  case tok::kw_for:                 // C99 6.8.5.3: for-statement
	...
	////
	case tok::kw_parfor:                 // C99 6.8.5.3: for-statement
		return ParseParForStatement(AttrList.take());
	////
...
StmtResult Parser::ParseParForStatement(AttributeList *Attr) {
  // FIXME: Use attributes?
  delete Attr;

  bool isParFor = true;
  assert(Tok.is(tok::kw_parfor) && "Not a for stmt!");
  SourceLocation ForLoc = ConsumeToken();  // eat the 'for'.
  ...
  if (!ForEach)
    return Actions.ActOnParForStmt(ForLoc, LParenLoc, FirstPart.take(), SecondPart,
                                SecondVar, ThirdPart, RParenLoc, Body.take());
}
